...
void watchdog_v1_test(void) {
    ...
    int action;
    char message[100];
    sprintf("%s%c%s%c%s", UC_COMMAND_BLINK_LED, UC_MESSAGE_DELIMITER, "", UC_MESSAGE_DELIMITER, "");

    while (1) {

// Read UART and wait for command.
#ifdef MASTER
        uart_comms_transmit_message(UC_COMMAND_BLINK_LED, "");
#endif
#if SLAVE
        uart_comms_receive_command(&action, message);
#endif

        if (action == UC_COMMAND_NONE) {
            continue;
        }
        ...
        // if (action == UC_COMMAND_CAPTURE_IMAGE) {
