...
uint8_t sd_card_create_path(char* folderPath, char* msg) {
    ...
    if (chars_get_num_bytes(folderPath) > MAX_PATH_LENGTH) {
        sprintf(msg, "Folder path > 50 characters\n");
        return FALSE;
    }
    ...
    for (int i = 0; folderPath[i] != '\0'; i++) {
        ...
        if (path[i] == '/' || folderPath[i + 1] == '\0') {
            ...
            if (fileFound) {
                ...
                if (file == NULL) {
                    sprintf(msg, "File %s could not be made", directory);
                    return FALSE;
                }
            ...
            if (stat(directory, &st) != 0 && mkdir(directory, 0700) != 0) {
                sprintf(msg, "Directory %s could not be made", directory);
                return FALSE;
            }
...
uint8_t sd_card_update_image_count(char* msg) {
    ...
    if (sd_card_create_path(DATA_FOLDER_PATH, msg) != TRUE) {
        return FALSE;
    }
    ...
    directory = opendir(DATA_FOLDER_PATH_START_AT_ROOT);
    if (directory == NULL) {
        sprintf(msg, "Img dir could not open");
        return FALSE;
    }
...
uint8_t sd_card_save_image(uint8_t* imageData, int imageLength, dt_datetime_t* Datetime, float temperature,
    ...
    sprintf(datetimeString, "%i%s%i%s%i_%s%i%s%i_%ip%i", Datetime->Date.year - 2000,
            Datetime->Date.month < 10 ? "0" : "", Datetime->Date.month, Datetime->Date.day < 10 ? "0" : "",
            Datetime->Date.day, Datetime->Time.hour < 10 ? "0" : "", Datetime->Time.hour,
            Datetime->Time.minute < 10 ? "0" : "", Datetime->Time.minute, (int)temperature,
            (int)(temperature * 1000.0) - (((int)temperature) * 1000));

    // Convert the decimal point of the temperature to a 'p' so there are no additonal periods in the filename.
    uint8_t length             = chars_get_num_bytes(datetimeString);
    datetimeString[length - 4] = 'p';
    ...
    if (imageFile == NULL) {
        sprintf(returnMsg, "Image %s file failed to open", filePath);
        return FALSE;
    }
